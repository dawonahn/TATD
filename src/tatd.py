...
class Kernel(nn.Module):
    ...
    def __init__(self, window, sigma=0.5):
        super().__init__()
        self.sigma = sigma
        self.window = window
...
class Tatd(nn.Module):
    ...
    def __init__(self, nmode, ndim, tmode, density, rank, window, sparse):
        ...
        self.smooth = Kernel(window).to(DEVICE)
        self.rank = rank
        self.tmode = tmode
        self.sparse = sparse
